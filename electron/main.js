...
function createBrowserView() {
    ...
    // セッション設定は defaultSession に統一

    // Anti-detection を最小限に（webdriver のみ）
    browserView.webContents.on('dom-ready', () => {
        browserView.webContents.executeJavaScript(`(function(){try{Object.defineProperty(navigator,'webdriver',{get:()=>undefined,configurable:true});}catch(e){}})();`).catch(() => { });
    });
...
function createWindow() {
    ...
    // 不要なグローバルCSPの上書きは行わない（外部サイトのレスポンスを改変しない）

    // 現在のURLを読み込み
    mainWindow.loadURL(currentUrl);

    // webviewTag は使用しない構成（BrowserView のみ）
    ...
    // メインウィンドウへの注入も最小限（webdriver のみ）
    mainWindow.webContents.on('dom-ready', () => {
        mainWindow.webContents.executeJavaScript(`(function(){try{Object.defineProperty(navigator,'webdriver',{get:()=>undefined,configurable:true});}catch(e){}})();`).catch(() => { });
    });
